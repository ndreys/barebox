...
static int mdio_mux_gpio_probe(struct device_d *dev)
{
	...
	gpios_num = of_gpio_count(dev->device_node);
	if (gpios_num <= 0) {
	...
	s = xzalloc(sizeof(*s));
	s->gpios = xzalloc(gpios_num * sizeof(s->gpios[0]));
	...
	r = gpio_request_array(s->gpios, gpios_num);
	if (r < 0)
		goto free_gpios;
...
free_gpios:
	gpio_free_array(s->gpios, gpios_num);
free_mem:
	free(s->gpios);
	free(s);
	return r;
