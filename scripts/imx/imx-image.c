...
#define ___perror(line, x)    perror(__FILE__ ":" #line ":" x)
#define __perror(line, x)  ___perror(line, x)
#define perror(x)	    __perror(__LINE__, x)
...
int main(int argc, char *argv[])
{
	...
	uint32_t *bb_header;
	size_t sizeof_bb_header, header_len = HEADER_LEN, signed_hdmi_firmware_size = 0;
	...
	case 2:
		...
		if (data.signed_hdmi_firmware_file) {
			...
			signed_hdmi_firmware_size = roundup(signed_hdmi_firmware_size, 0x1a000);
		...
		barebox_image_size += add_header_v2(&data, buf + signed_hdmi_firmware_size);
		break;
	default:
