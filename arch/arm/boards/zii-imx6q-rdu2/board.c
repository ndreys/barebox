...
static int rdu2_fixup_disable_pic(struct device_node *root, void *context)
{
...
static int rdu2_disable_sp_node(void)
{
	...
	if (*disable_pic) {
		pr_info("PIC is disabled, run "
			...
			" to enable\n");
		of_register_fixup(rdu2_fixup_disable_pic, NULL);
	} else {
...
#define RDU2_PANEL_32P0	IT_DUAL_LVDS,   BF_JEIDA, "auo,p320hvn03"
